...
class CommandLineInterface {
  ...
  [finalize](start, error) {
    ...
    moan.log
      .writeln()
      .writeln(chalk.bold(`BUILD ${error ? 'FAILED' : 'SUCCESS'}`))
      .writeln()
      .writeln(`Total time: ${moment.utc(end.diff(start)).format('HH:mm:ss:SSS')}`)
      .writeln(`Finished at: ${end.format('ddd MMM DD HH:mm:SS z YYYY')}`)
      .writeln(`Final memory: ${numeral(memory.heapUsed).format('0b')}/${numeral(memory.heapTotal).format('0b')}`)
  }
